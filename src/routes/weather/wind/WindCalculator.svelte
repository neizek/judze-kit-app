<script lang="ts">
	...
	$: {
		shipsCourse, shipsSpeed, relativeWindDirection, relativeWindSpeed, calculateWind();
	}
</script>
	...
	<div class="vertical-flex space-l">
		...
		<EqualGrid --mobileColumnsQty="2" --tabletColumnsQty={2} --desktopColumnsQty="2">
			<FormItem label="Direction">
				<Input
					type="number"
					bind:value={relativeWindDirection}
					on:input={calculateWind}
					min={0}
					max={360}
					step={1}
					suffix="deg"
			...
			<EqualGrid --mobileColumnsQty={2} --tabletColumnsQty={2} --desktopColumnsQty={2}>
				<FormItem label="Direction">
					<span class="text-size-m">
						{String(Number(trueWindDirection).toFixed(0) ?? 0).padStart(3, '0')}&#176;
					</span>
				</FormItem>
				<FormItem label="Speed">
					<span class="text-size-m">
						{Number(!isNaN(trueWindSpeed) ? trueWindSpeed : 0).toFixed(1)} kn
					</span>
				</FormItem>
			</EqualGrid>
		</div>
