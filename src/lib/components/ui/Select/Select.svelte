<script lang="ts">
	...
	let multiSelectedItems: SelectItem[] = multiple
	...
	function addValue(item: SelectItem): void {
		if (multiple) {
			...
			multiSelectedItems = [...multiSelectedItems, item];
	...
	function removeValue(valueToRemove: any): void {
		...
		multiSelectedItems = multiSelectedItems.filter((item) => item.value !== valueToRemove);
	}
...
<div use:clickOutside on:click_outside={() => (open = false)} on:click={onClick} class="select">
	<label class="input extended" class:error={hasError}>
		...
		{#if clearable && !isEmptyValue(value) && !multiple}
			<!-- svelte-ignore a11y-click-events-have-key-events -->
			<!-- svelte-ignore a11y-no-static-element-interactions -->
			<i class="fa-solid fa-xmark clearIcon" on:click|preventDefault={onClear}></i>
		{/if}
		{#if hasError}
